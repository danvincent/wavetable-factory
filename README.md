...
- 🎚️ **Triple export formats** — 32-bit float WAV for Ableton, 16-bit PCM WAV for Polyend Tracker, and text `.txt` for pirate-synth
...
4. **Choose export target** — Ableton Live, Polyend Tracker, pirate-synth `.txt`, Both, or All
...
| **pirate-synth** | text `.txt` (one float sample per line) | N/A | text float | generated from full wavetable sample stream | N/A |
...
<library-path>/
  ...
  pirate/     ← text `.txt` files for pirate-synth
```

> ⚠️ `pirate-synth` `.txt` files in this project are plain-text numeric wavetables (matching `danvincent/pirate-synth`), and are **not** Bitwig/Surge binary `.wt` (`vawt`) files.
...
│   │   ├── exporter.js          # Wavetable exporters (Ableton WAV, Polyend WAV, pirate-synth .txt)
